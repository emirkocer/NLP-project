...
math_prompt = """Below is a math problem. Please provide a solution and final answer to the problem.
### Problem:
{}
...
def formatting_prompts_func(batch):
    problems = [item['problem'] for item in batch['data']]
    solutions = [item['solution'] for item in batch['data']]
    answers = [item['answer'] for item in batch['data']]
    texts = []
    for problem, solution, answer in zip(problems, solutions, answers):
        text = math_prompt.format(problem, solution, answer) + EOS_TOKEN
        texts.append(text)
    return {"text": texts}

# Ensure your dataset is properly loaded as a Dataset object
from datasets import load_dataset, Dataset
import json
...
train_dataset = load_jsonl_to_dataset('math_train.jsonl')
...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        per_device_train_batch_size = 16, # adjusted based on the memory usage
